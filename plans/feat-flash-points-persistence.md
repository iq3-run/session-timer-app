...
- 永続化フォーマットはJSON配列（`jsonEncode(points)`/`jsonDecode`で`List<int>`
  ...
  作らない。
- **初回起動と「ユーザーが全削除した空リスト」の区別**：`prefs.getString(key)`
  が`null`（キーが一度も書き込まれていない）の場合のみ
  `defaultCompletionFlashPointsMinutes`をシードして`build()`内で即座に
  永続化する（`TimeTargetsController.build()`が`_persist`を呼ぶのと同じ
  タイミング）。それ以外（キーが存在し、中身が空配列`[]`）は「ユーザーが
  意図的に全削除した」として空リストのまま返す。
